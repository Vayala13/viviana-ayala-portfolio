    ...
    <section id="opensource" className="py-24 bg-[#0a0a0f] relative overflow-hidden">
      ...
      <div className="container relative">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-14">
          {stats.map((stat) => (
            <div key={stat.label} className="bg-[#111827] border border-[#1e2d3d] rounded-lg p-5 text-center hover:border-[#00ff88]/30 transition-colors duration-200 relative overflow-hidden">
              <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-[#00ff88]/30 to-transparent" />
              <div className="text-2xl font-bold text-[#00ff88] neon-text-glow mb-1">{stat.value}</div>
        ...
        <div className="space-y-3 pl-4 border-l border-[#1e2d3d] relative">
          {contributions.map((contrib, i) => (
            <a key={i} href={contrib.url} target="_blank" rel="noopener noreferrer"
              className="block bg-[#111827] border border-[#1e2d3d] rounded-lg p-6 hover:border-[#00ff88]/40 transition-all duration-200 group relative">
              {/* Node */}
              <div className="absolute -left-[1.1rem] top-6 w-2 h-2 rounded-full bg-[#0a0a0f] border border-[#00ff88]/40 group-hover:border-[#00ff88] transition-colors" />
              <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <span className="font-mono text-xs text-[#475569]">{contrib.org} /</span>
                    <span className="font-semibold text-[#e2e8f0] group-hover:text-[#00ff88] transition-colors duration-150">{contrib.project}</span>
        ...
        <div className="mt-10 pl-4">
          <a href="https://github.com/Vayala13" target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-2 w-fit px-6 py-3 border border-[#1e2d3d] text-[#64748b] font-mono text-sm rounded hover:border-[#00ff88]/50 hover:text-[#00ff88] transition-all duration-150 btn-press">
