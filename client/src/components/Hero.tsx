 ...
 * Design: Black canvas, soft pink (#FFB7C5), Space Grotesk + JetBrains Mono.
 */
...
export default function Hero() {
      ...
      for (let i = 0; i < NODES; i++) {
        for (let j = i + 1; j < NODES; j++) {
          ...
          if (dist < 150) {
            ...
            ctx.strokeStyle = `rgba(255, 183, 197,${0.15 * (1 - dist / 150)})`; ctx.lineWidth = 0.6; ctx.stroke();
          }
        }
      }
      nodes.forEach((n) => {
        ctx.beginPath(); ctx.arc(n.x, n.y, n.r, 0, Math.PI * 2);
        ctx.fillStyle = "rgba(255, 183, 197,0.55)"; ctx.fill();
      });
  ...
  return (
    ...
    >
      ...
      {/* Grid */}
      <div className="absolute inset-0 opacity-[0.035]" style={{ backgroundImage: `linear-gradient(rgba(255, 183, 197,0.6) 1px, transparent 1px), linear-gradient(90deg, rgba(255, 183, 197,0.6) 1px, transparent 1px)`, backgroundSize: "60px 60px" }} />
      {/* Left accent rail */}
      <div className="absolute left-0 top-0 bottom-0 w-px bg-gradient-to-b from-transparent via-[#FFB7C5]/20 to-transparent hidden lg:block" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-12 items-center">
          ...
          <div>
            ...
            <h1 className="text-6xl md:text-8xl font-bold text-[#e2e8f0] leading-[1.0] mb-5 animate-fade-in-up" style={{ fontFamily: "'Space Grotesk', sans-serif", animationDelay: "80ms" }}>
              Viviana<br />
              <span className="text-[#FFB7C5] neon-text-glow">Ayala.</span>
            </h1>
            ...
            <div className="flex flex-wrap gap-4 mb-14 animate-fade-in-up" style={{ animationDelay: "320ms" }}>
              <a href="#projects" className="px-6 py-3 bg-[#FFB7C5] text-[#0a0a0f] font-semibold text-sm rounded hover:bg-[#F5A0B0] transition-colors duration-150 btn-press">
                See My Work
              </a>
              <a href="https://github.com/Vayala13" target="_blank" rel="noopener noreferrer" className="px-6 py-3 border border-[#1e2d3d] text-[#e2e8f0] font-semibold text-sm rounded hover:border-[#FFB7C5]/50 hover:text-[#FFB7C5] transition-all duration-150 btn-press flex items-center gap-2">
            ...
            <div className="animate-fade-in-up" style={{ animationDelay: "400ms" }}>
              ...
              <div className="flex flex-wrap gap-3">
                {techStack.map((tech) => (
                  <div key={tech.name} className="flex items-center gap-2 px-3 py-2 bg-[#111827]/80 border border-[#1e2d3d] rounded hover:border-[#FFB7C5]/40 transition-colors duration-150 group">
          ...
          <div className="hidden lg:flex flex-col gap-4 animate-fade-in-up" style={{ animationDelay: "480ms" }}>
            {[
              ...
              <div key={item.label} className={`p-4 rounded border ${item.accent ? "bg-[#FFB7C5]/5 border-[#FFB7C5]/20" : "bg-[#111827]/60 border-[#1e2d3d]"}`}>
                <div className="font-mono text-[10px] tracking-widest uppercase text-[#475569] mb-1">{item.label}</div>
                <div className={`font-mono text-sm ${item.accent ? "text-[#FFB7C5]" : "text-[#e2e8f0]"}`}>{item.value}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="absolute bottom-8 left-8 hidden lg:flex flex-col items-center gap-2 opacity-30">
        <div className="w-px h-12 bg-gradient-to-b from-[#FFB7C5] to-transparent" />
